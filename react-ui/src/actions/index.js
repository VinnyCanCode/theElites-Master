...
export const searchTerm = (value) => async (dispatch) => {
  try {
    const res = await axios.get(`http://localhost:5000/products/all/${value}`);
  ...
  try {
    ...
    if (gen.length === 0) {
      const res = await axios.get(
        `http://localhost:5000/products/${searchTerm}?sortBy=${sort}&byColor=${colorFilter}&byBrand=${brandFilter}&byPrice=${priceFilter}`
      );
    ...
    if (searchTerm === 'new') {
      const res = await axios.get(
        `http://localhost:5000/products/${gen}?prod=${searchTerm}&sortBy=${sort}&byColor=${colorFilter}&byBrand=${brandFilter}&byPrice=${priceFilter}`
      );
...
export const selectedItem = (routeID) => async (dispatch) => {
  try {
    const res = await axios.get(`http://localhost:5000/product/${routeID}`);
...
export const addToWishlist = (itemId) => async (dispatch) => {
  ...
  try {
    const res = await axios.post(`http://localhost:5000/wishlist/${itemId}`);
...
export const removeFromWishlist = (itemId) => async (dispatch) => {
  ...
  try {
    const res = await axios.patch(
      `http://localhost:5000/wishlist-remove/${itemId}`
    );
...
export const addItemToCart = (itemId) => async (dispatch) => {
  ...
  try {
    const res = await axios.post(`http://localhost:5000/cart/${itemId}`);
...
export const deleteItemFromCart = (itemId) => async (dispatch) => {
  ...
  try {
    const res = await axios.patch(`http://localhost:5000/cart/${itemId}`);
...
export const subtractOneFromQuantity = (itemId) => async (dispatch) => {
  ...
  try {
    const res = await axios.patch(`http://localhost:5000/cart/minus/${itemId}`);
